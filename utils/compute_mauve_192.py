...
def parse_text(reference_text, prediction_text, tokenizer):
    ...
    reference_tokens = reference_tokens[:192]#reference_tokens[:min_len]
    reference_text = decode(reference_tokens, tokenizer)
    prediction_tokens = prediction_tokens[:192]#prediction_tokens[:min_len]
    prediction_text = decode(prediction_tokens, tokenizer)
    '''
        Truncating the text to maximum token of 192 accoridng to the author's implementation:
        ...
        Only evaluate instances with exact 192 tokens length based on the author's implementation:
    ...
    '''
    if min(len(reference_tokens), len(prediction_tokens)) == 192:
        flag = True
    else:
        flag = False
